...
int main(int argc, char** argv)
{
  ...
  // mashkin::forwardConstIterator< size_t > citer = test.cbegin();
  // mashkin::forwardIterator< size_t > iter = test.begin();
  // mashkin::forwardIterator< size_t > last = test.begin();
  // last++;
  // last++;
  // last++;
  // last++;
  // mashkin::forwardIterator< size_t> before_iter = test.before_begin();
  // mashkin::forwardConstIterator< size_t > cend = test.cend();
  // mashkin::forwardIterator< size_t > end = test.end();
  // mashkin::forwardIterator< size_t > Iter = test.erase_after(before_iter);
